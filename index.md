...
> **Under construction.** Scaffolding is in progress — see the
> [issues](https://github.com/ACCIDDA/imurun/issues).
...
Part of the `imu*` family: **imuGAP** (the model), **imumap**
(geospatial / dashboard export), and **imurun** (this — the data-file
CLI).

## What it does

- Reads imuGAP model inputs from data files — either a directory of
  `observations` / `populations` / `locations` as **CSV or RDS**, or a
  single **Excel workbook** with one sheet per input.
...
The Excel workbook is **one** supported input mode — convenient for
people who don’t work in R — not the whole package. The same CLI takes
plain CSV/RDS too, so it fits both spreadsheet users and scripted
pipelines.
...
([\#7](https://github.com/ACCIDDA/imurun/issues/7)); the Excel input
mode, the template, and the friendly validation layer build on top of
it.
