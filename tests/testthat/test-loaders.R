...
test_that("read_inputs returns all three inputs", {
  ...
  write.csv(data.frame(positive = 1:2, sample_n = 3:4),
            file.path(dir, "observations.csv"), row.names = FALSE)
  write.csv(data.frame(a = 1:2), file.path(dir, "populations.csv"), row.names = FALSE)
  saveRDS(data.frame(loc_id = 1, parent_id = NA),
