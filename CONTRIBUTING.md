...
imports [imuGAP](https://github.com/ACCIDDA/imuGAP), which installs as an
ordinary dependency (a binary where one is available).
...
|----------|-------------|
| **R-CMD-check** | Runs `R CMD check --as-cran` on Ubuntu, macOS, and Windows across R release, oldrel, and devel (9 jobs total). R-devel jobs are allowed to fail without blocking the PR. |
